...
export const paymentAPI = {
  ...
  initialize: async (amount: number, currency: string, paymentMethod: string, bankName?: string, accountNumber?: string, accountName?: string) => {
...
export const adminAPI = {
  login: async (username: string, password: string) => {
    console.log('adminAPI.login called with:', { username, hasPassword: !!password });
  ...
  acceptContestant: async (roundNumber: number, userId: string) => {
    const response = await api.post(`/admin/rounds/${roundNumber}/accept/${userId}`)
    return response.data
  },
